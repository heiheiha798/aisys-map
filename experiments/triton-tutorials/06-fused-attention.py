...
这是一个用 Triton 实现的 Flash Attention v2 算法版本，
对应 Tri Dao 的论文：
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # 当前 stage 负责处理的 value 范围。
    if STAGE == 1:
        lo, hi = 0, start_m * BLOCK_M
    elif STAGE == 2:
        lo, hi = start_m * BLOCK_M, (start_m + 1) * BLOCK_M
        lo = tl.multiple_of(lo, BLOCK_M)
    # `causal = False` 时，直接覆盖整段上下文。
    else:
        lo, hi = 0, N_CTX
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        p = p.to(dtype)
        # 注意：这种 FP8 下非转置的 v 只在 Blackwell 上受支持。
        acc = tl.dot(p, v, acc)
        # 更新 m_i 和 l_i。
        # 放在 loop 末尾有助于减小 register pressure。
        l_i = l_i * alpha + l_ij
        m_i = m_ij
        offsetk_y += BLOCK_N
        offsetv_y += BLOCK_N
...
def prune_invalid_configs(configs, named_args, **kwargs):
    ...
    # 过滤掉 BLOCK_M > N_CTX 的 config；
    # 另外在 causal=True 时，也过滤掉 BLOCK_M < BLOCK_N 的 config。
    return [
...
def _attn_fwd(sm_scale, M,  #
    ...
    # 读取 q；它会在整个计算过程中常驻 SRAM。
    q = desc_q.load([qo_offset_y, 0])
    # stage 1: off-band 部分。
    # 当 causal=True 时，STAGE=3，而 `_attn_fwd_inner` 收到的 STAGE 是 1；
    # 当 causal=False 时，STAGE=1，而 `_attn_fwd_inner` 收到的 STAGE 是 3。
    if STAGE & 1:
...
# 下面是计算 dK 和 dV 的主 inner-loop 逻辑。
@triton.jit
def _attn_bwd_dkdv(dk, dv,  #
    ...
    # BLOCK_N1 必须是 BLOCK_M1 的整数倍，否则这段代码不成立。
    tl.static_assert(BLOCK_N1 % BLOCK_M1 == 0)
    curr_m = start_m
    step_m = BLOCK_M1
    for blk_idx in range(num_steps):
        qT = tl.load(qT_ptrs)
        # 在计算 qk 前先读取 m，以减少 pipeline stall。
...
def _attn_bwd_dq(dq, q, K, V,  #
    ...
    # D（也就是 delta）已经预先除过 ds_scale。
    Di = tl.load(D + offs_m)
    # BLOCK_M2 必须是 BLOCK_N2 的整数倍，否则代码不会正确工作。
    ...
    for blk_idx in range(num_steps):
        ...
        ds = ds.to(tl.float16)
        # 计算 dQ。
        # NOTE: 最后需要对 dq 做 de-scale，因为 kT 在前面已经预缩放过。
...
def _attn_bwd(Q, K, V, sm_scale,  #
    ...
    # 读取 K 和 V；它们会在整个 inner loop 中常驻 SRAM。
    ...
    if CAUSAL:
        # 对 masked（对角）blocks 计算 dQ。
        # NOTE: 这里扫描 QK^T 每一行的方向比较绕：
        # 整体上是从右往左，但单次 `_attn_bwd_dq` 调用内部又是从左往右。
        # 这并没有什么特别深的数学含义，只是为了尽量复用上面 dK / dV 的 loop 结构。
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, warp_specialize=True):
        ...
        extra_kern_args = {}
        # 针对 AMD target 的 tuning。
        if is_hip():
            waves_per_eu = 3 if HEAD_DIM_K <= 64 else 2
            extra_kern_args = {"waves_per_eu": waves_per_eu, "allow_flush_denorm": True}

        M = torch.empty((q.shape[0], q.shape[1], q.shape[2]), device=q.device, dtype=torch.float32)
        # 在 Hopper + warpspec 情况下使用 device_descriptor。
        if supports_host_descriptor() and not (is_hopper() and warp_specialize):
            # 注意：在 Hopper 上，FP8 dot 的第二个输入 tensor 不能是非转置形式。
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, dtype=torch.float16):
    ...
    rtol = 0.0
    # 这是针对 CDNA2 已知硬件限制的 relative tolerance workaround。
    # 细节见：
...
TORCH_HAS_FP8 = hasattr(torch, 'float8_e5m2')
BATCH, N_HEADS = 4, 32
# 固定 head 和 batch=4，只改变 seq length。
configs = []
for HEAD_DIM in [64, 128]:
    for mode in ["fwd", "bwd"]:
        for causal in [True, False]:
            # 在 Hopper 上为 causal forward 打开 warpspec。
