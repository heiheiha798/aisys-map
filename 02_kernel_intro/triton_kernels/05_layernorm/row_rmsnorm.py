...
@triton.jit
def row_rmsnorm_kernel(x_ptr, y_ptr, rows, cols, stride, eps, BLOCK_SIZE: tl.constexpr):
    row = tl.program_id(0)
    if row >= rows:
        return
...
def cpu_row_rmsnorm(x: torch.Tensor, eps: float) -> torch.Tensor:
...
def make_input(rows: int, cols: int, device: str) -> torch.Tensor:
    ...
    x = torch.sin((r + 5.0) * 0.009) - torch.cos((c + 7.0) * 0.017)
    y = (((r * 13.0 + c * 5.0) % 29.0) - 14.0) * 0.04
    return 0.8 * x + y
...
def run() -> None:
    ...
    grid = (rows,)
    row_rmsnorm_kernel[grid](x, y, rows, cols, x.stride(0), EPS, BLOCK_SIZE=block_size)

    ref = cpu_row_rmsnorm(x, EPS)
    max_abs_diff = torch.max(torch.abs(y - ref)).item()
    ok = max_abs_diff < 2e-4

    if ok:
        print(
            f"row_rmsnorm passed. rows={rows}, cols={cols}, "
        ...
        raise AssertionError(
            f"row_rmsnorm failed. max_abs_diff={max_abs_diff}, idx={idx}, got={got}, ref={expected}"
        )
